function [ t_test_result, p_vals, t_statistics, t_threshold, hplot ] ...
         = permutationRegress( Y, X, C, group, varargin )
% function [  t_test_result, p_vals, t_statistics, t_threshold, hplot ]  ...
%                  = permutationRegress( Y, X, C,  GROUP,  ['param',value]...)
%
% calculates permuted t-statistics using regress() so it can handle categoricals.
%
% DATA       =  Y( TRIAL, SAMPLE )     
...
%   [~,varb,t_wg] = ols(Y,pX,C); % T_WG ( CONTRAST, SAMPLE )
  t_wg = apply(1, @(y) lmeEEG_regress(y, pX), Y); 
  t_wg(1,:) = []; % remove intercept
...
t_wg = apply(1, @(y) lmeEEG_regress(y, X), Y);
t_wg(1,:) = []; % remove intercept
...
% for each effect (i.e. terms, so across levels of categorical)
%   make contrast matrix for FE = L (e.g. [0 1 1])
%   test H0: L*beta=e
%     r = size(L,1); % rank
%     delta = (L*beta - e);
%     F = delta'*( (L*covbeta*L') \ delta )/r;
%     DF2 = N - size(L,2);
%     DF1 = r;
%     P = fcdf(F,DF1,DF2,'upper');
%
% I think ols() outputs can be used to calculate this
% cope is contrasts of param estimates, and varcope is the variation
% it can also give covb
% and I think tc is already L
% actually, I think that it can already do this but using the contrasts
% setting?
% anova() uses [0 1 0; 0 0 1] to compare both contrasts within a factor
% (0=intercept)
% L = C;
% r = size(L,1);
% delta = L*cope;
% F = delta'*( (L*covb*L') \ delta) / r;
% DF1 = r;
% DF2 = size(data,1) - size(data,2);
% P = fcdf(F,DF1,DF2,'upper');

% doesn't work, as covb has zeros on [2 3] and [3 2], which means F is
% miniscule. doesn't seem to be due to RE, as fitglme without RE still
% gives similar anova() effects+coeffcovar
